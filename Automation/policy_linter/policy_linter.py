...
class PolicyLinter:
    ...
    def normalize_pattern(self,pattern):
        # Remove attribute specifications like ${...} and handle tags/views after ;
        ...
        if ';' in pattern:
            ...
            if re.search(resource_regex, pattern):
                # Remove  the tag portion between ; and :
                pattern = re.sub(r';[^:]*:', ':', pattern)
            else:
                # No clear resource continuation, remove everything after ;
                # simple approach handles cases like proj/*:env/*;tags or proj/*:flag/*;view:frontend
                pattern = re.sub(r';.*$', '', pattern)
    ...
    def generate_limited_update_policy_role(self, fixed_policies:list)->None:
        ...
        ld_policy=[
            {
                "resources": [f"role/{policy}" for policy in fixed_policies],
